...
//       try {
//         const res = await fetch("/api/get-passwords", { credentials: "include" });
//         const data = await res.json();

//         if (res.ok) setPasswords(data);
//         else console.error("Failed to fetch passwords:", data.error);
//       } catch (err) {
...
export default function YourPassword() {
  ...
  const deletePassword = async (id) => {
    ...
    try {
      const res = await fetch(`/api/delete-password?id=${id}`, {
        method: "DELETE",
        credentials: "include",
      });
              ...
              >
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
                    <span className="text-slate-400">Password</span>
                    <span className="font-mono text-white">
                      {maskPassword(p.password, showPasswords[p._id])}
                    </span>
                  </div>
                  {p.website && (
                    <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
